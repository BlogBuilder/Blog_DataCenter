<template>
    <div class="card card-transparent">
        ...
        <div class="card-block">
            <div class="row">
                <div class="col-md-10">
                    <button class="btn btn-info" type="button" onclick='window.location.href="/tag/create"'>
                            ...
                            <tr role="row" class="odd text-center">
                                ...
                                <td class="v-align-middle">
                                    ...
                                    <router-link class="btn btn-primary btn-sm"
                                                 :to="{ path: '/tag/update/'+item.id}">修 改
                                    </router-link>
...
<script type="es6">
    module.exports = {
        ...
        methods: {
            ...
            _fetchData(){
                let me = this;
                me.$http.get("/api/v1.0/admin/tag/list/" + me.currentPage, {
                    params: me.condition
                }).then(response => {
            ...
            _batchDestroy(){
                ...
                confirm("是否批量删除选中对象？", () => {
                    me.$http.delete("/api/v1.0/admin/tag/batchDestroy/" + me.selected).then(response => {
                        let data = response.data;
                        codeState(data.code, {
                            200(){
                                alert("删除成功！");
            ...
            _destroyData(item){
                ...
                if (item) {
                    confirm("是否删除" + item.name + "？", () => {
                        me.$http.delete("/api/v1.0/admin/tag/destroy/" + item.id).then(response => {
                            let data = response.data;
                            codeState(data.code, {
                                200(){
                                    alert("删除成功！");
